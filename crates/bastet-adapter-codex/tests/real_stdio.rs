...
fn installed_codex_lists_models_over_stdio() {
    ...
    let mut server = adapter.connect_app_server(Duration::from_secs(15)).unwrap();
...
#[test]
#[ignore = "requires explicit Codex CLI and isolated empty root; writes one bounded fixture file"]
fn installed_codex_reports_a_bounded_workspace_write_over_stdio() {
    let executable = PathBuf::from(
        std::env::var_os("BASTET_CODEX_BINARY").expect("BASTET_CODEX_BINARY must be set"),
    );
    let probe_root = PathBuf::from(
    ...
    assert_eq!(entries[0].file_name(), "receipt.txt");
    assert_eq!(
